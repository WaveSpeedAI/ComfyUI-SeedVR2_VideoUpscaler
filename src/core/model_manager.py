...
def configure_runner(
    ...
    if cached_runner and block_swap_config and block_swap_config.get("cache_model", False):
        ...
        if blockswap_needed:
            ...
            if has_cached_config:
                ...
                config_matches = (
                    cached_config.get("blocks_to_swap") == block_swap_config.get("blocks_to_swap")
                    and cached_config.get("offload_io_components")
                    == block_swap_config.get("offload_io_components", False)
                    and cached_config.get("use_non_blocking") == block_swap_config.get("use_non_blocking", True)
                )
    ...
    runner = configure_vae_model_inference(
        runner, device, checkpoint_path, config, preserve_vram, model_weight, vram_info, debug, block_swap_config
    )
    if debug:
    ...
    blockswap_active = block_swap_config and block_swap_config.get("blocks_to_swap", 0) > 0
...
def configure_dit_model_inference(
    runner,
    device,
    checkpoint,
    config,
...
def configure_vae_model_inference(
    runner,
    device,
    checkpoint_path,
    config,
